    ...
    pub fn print_report(&self, source: &str) {
        ...
        if let Ok(output_path) = std::env::var(PROFILE_OUTPUT_ENV) {
            match std::fs::File::create(&output_path) {
                ...
                Err(e) => {
                    eprintln!(
                        "Warning: Could not write profile output to {}: {}",
                        output_path, e
                    );
                }
            }
    ...
    fn write_summary(&self, w: &mut dyn Write) {
        ...
        if total_count > 0 {
            ...
            if !self.total_time.is_zero() {
                let instr_per_sec = total_count as f64 / self.total_time.as_secs_f64();
                writeln!(
                    w,
                    "  Instructions per second:     {:.2}M",
                    instr_per_sec / 1_000_000.0
                )
                .ok();
            }
        }
